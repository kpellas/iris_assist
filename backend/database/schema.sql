...
-- pgvector extension is created in 001_init_pgvector.sql
...
-- Note: For production with >1M vectors, consider:
-- SET max_parallel_maintenance_workers = 7;
-- SET maintenance_work_mem = '2GB';
-- CREATE INDEX IF NOT EXISTS idx_memories_embedding ON memories USING ivfflat (embedding vector_cosine_ops) WITH (lists = 100);
-- For now, create basic index (will be optimized when more data exists)
...
CREATE INDEX IF NOT EXISTS idx_protocol_runs_protocol_id ON protocol_runs(protocol_id);
...
CREATE INDEX IF NOT EXISTS idx_display_states_user_id ON display_states(user_id);
